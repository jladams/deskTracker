...
output$ovDayLines <- renderPlotly({
  ...
  p <- p +
    ...
    geom_text(aes(label = round(value, 2), color = weekday), vjust = -1) + 
...
output$modeHeat <- renderPlot({plotHeat(varData("mode"))})
...
```{r}
output$modeData <- renderTable({varData("mode")})
...
output$typeHeat <- renderPlot({plotHeat(varData("type"))})
...
output$typevarData <- renderTable({varData("type")})
tableOutput("typevarData")
```
...
output$refHeat <- renderPlot({plotHeat(varData("referral"))})
...
output$refvarData <- renderTable({varData("referral")})
tableOutput("refvarData")
```
