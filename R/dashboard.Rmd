...
output$modeHeat <- renderPlot({
  ...
  p <- p +
    ...
    scale_fill_gradient(low="mistyrose", high="navy", guide=guide_colorbar(title=expression(paste("Number of Interactions")))) +
...
output$heatData <- renderTable({df_mode()})
tableOutput("heatData")
```
...
func <- reactive({ func <- match.fun(as.character(input$calculation)) })
